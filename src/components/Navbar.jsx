                ...
                >
                    <div className="flex justify-between items-center mx-auto px-2 sm:px-3 md:px-4 lg:px-4">
                        ...
                        <div
                            ...
                            id="mobile-menu-2"
                        >
                            <ul className="flex flex-col items-center justify-center w-full min-h-[82vh] lg:min-h-0 transition-all duration-300 ease-in-out space-y-4 lg:space-y-0 lg:flex-row lg:space-x-5 xl:space-x-7 2xl:space-x-8 lg:w-auto lg:bg-transparent p-6 lg:p-0 rounded-2xl lg:rounded-none" style={{ backgroundColor: 'var(--surface-black)' }}>
                                    ...
                                    <li key={idx}>
                                        {item.disabled ? (
                                            <span
                                                ...
                                                aria-disabled="true"
                                                tabIndex={-1}
                                                className="opacity-50 cursor-not-allowed relative block py-2 pr-4 pl-3 duration-200 lg:p-1 transform rounded-md text-sm sm:text-base lg:text-sm xl:text-sm whitespace-nowrap"
                                            ...
                                            <NavLink
                                                ...
                                                className={({ isActive }) =>
                                                    ...
                                                    lg:after:w-full lg:after:origin-left lg:after:scale-x-0 lg:after:transition-transform 
                                                    lg:after:duration-300 lg:hover:after:scale-x-100
                                                    hover:bg-white/10 lg:hover:bg-transparent
                                                    ${isActive ? "" : ""}`
                                                }
                                                style={({ isActive }) => ({
                                                    color: isActive ? 'var(--yellow-primary)' : 'var(--gray-text)',
                                                })}
                                            >
                                                {item.name}
                                            </NavLink>
                                ...
                                {navItems.filter(item => item.isCTA).map((item, idx) => (
                                    <li key={`cta-${idx}`} className="lg:hidden mt-6">
                                        <NavLink
                                            to={`/${item.value}`}
                                            onClick={closeMenu}
                                            className="font-medium rounded-full text-sm px-5 py-2 transition-all duration-200 hover:scale-95"
                                            style={{
                                                borderWidth: '1px',
                                                borderStyle: 'solid',
                        ...
                        <div className="flex items-center">
                            <div className='mr-4 lg:mr-5 xl:mr-6 hidden lg:inline-block text-xs xl:text-sm' style={{ color: 'var(--gray-text)' }}>
                                {(() => {
                                    ...
                                    return (
                                        <>
                                            <span>{day}</span>
                                            <span className='mx-2 font-bold' style={{ color: 'var(--yellow-primary)' }}>//</span>
                                            <span>{month}</span>
                                            <span className='mx-2 font-bold' style={{ color: 'var(--yellow-primary)' }}>//</span>
                                            <span>{year}</span>
                                        </>
                            ...
                            {navItems.filter(item => item.isCTA).map((item, idx) => (
                                <NavLink
                                    key={`cta-desktop-${idx}`}
                                    to={`/${item.value}`}
                                    className="hidden lg:block font-medium rounded-full text-xs xl:text-sm px-3 xl:px-4 py-1.5 xl:py-2 transition-all duration-200 hover:scale-95 whitespace-nowrap"
                                    style={{
                                        borderWidth: '1px',
                                        borderStyle: 'solid',
                                        borderColor: 'var(--yellow-primary)',
                            ...
                            >
                                ...
                                <div className="relative w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center">
                                    <span className={`absolute h-0.5 w-4 sm:w-5 transform transition-all duration-300 ease-in-out 
